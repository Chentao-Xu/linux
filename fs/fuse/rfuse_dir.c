			   ...
			   struct fuse_setattr_in *arg, bool trust_local_cmtime)
{
	...
	if (ivalid & ATTR_SIZE)
		{
			if(arg==NULL){
				printk("fffarg==NULL no fenpei\n");
			}
			//printk("fffarg->valid fuzhi\n");
			arg->valid |= FATTR_SIZE,   arg->size = iattr->ia_size;
			//printk("fffarg->valid fuzhi end\n");
		}
...
static void rfuse_fillattr(struct inode *inode, struct fuse_attr *attr, struct kstat *stat)
{
	...
	//printk("bbbrfuse_fillattrstatstart");
	

	/* see the comment in fuse_change_attributes() */
	if (fc->writeback_cache && S_ISREG(inode->i_mode)) {
		//printk("bbbfc->writeback_cache");
...
int rfuse_do_getattr(struct inode *inode, struct kstat *stat, struct file *file){
	...
	u64 attr_version;
	//printk("aaarfuse_do_getattrstart\n");
	...
	if(!err){
		...
		inode_wrong_type(inode, outarg->attr.mode)){
			//printk("aaarfuse_do_getattrbad");
			fuse_make_bad(inode);
			err = -EIO;
		}
		else{
			//printk("aaarfuse_do_getattrokk");
			fuse_change_attributes(inode, &outarg->attr,rfuse_attr_timeout(outarg),attr_version);
			if(stat){
				//printk("aaarfuse_do_getattrneedstat");
...
int rfuse_do_setattr(struct dentry *dentry, struct iattr *attr, struct file *file){
	...
	//printk("dddsetattrstart\n");

	if (!fc->default_permissions)
		attr->ia_valid |= ATTR_FORCE;

	//printk("dddsetattr_preparestart\n");
	err = setattr_prepare(&nop_mnt_idmap, dentry, attr);
	//printk("dddsetattr_prepareend\n");
	if (err)
	{
		//printk("dddsetattr_prepare error\n");
		return err;
	}
	if (attr->ia_valid & ATTR_SIZE) {
		if (WARN_ON(!S_ISREG(inode->i_mode)))
			return -EIO;
		is_truncate = true;
		//printk("dddsetattr_prepare attr->ia_valid & ATTR_SIZE end\n");
	}
	...
	if (attr->ia_valid & ATTR_OPEN) {
		...
		if (fc->atomic_o_trunc) {
			//printk("dddsetattr_prepare attr->ia_valid & ATTR_OPEN start\n");
			/*
			...
			//printk("dddsetattr_prepare attr->ia_valid & ATTR_OPEN end\n");
			goto out;
		}
		file = NULL;
	}
	...
	//printk("dddsetattr_beforerfuse_get_req error\n");


	r_req = rfuse_get_req(fm, false, false);
	if(r_req ==	NULL)
	{
		printk("fffrfuse_do_setattr r_req ==NULL\n");
	}
	//printk("fffrfuse_do_setattr r_req ==%p \n",r_req);
	inarg = (struct fuse_setattr_in*)(&r_req->args);
	//printk("fffsizeof(fuse_setattr_in)=%zu, sizeof(args.argument_space)=%zu\n",
       //sizeof(struct fuse_setattr_in), sizeof(r_req->args.argument_space));
	...
	//printk("dddsetattr_afterend\n");
	rfuse_put_request(r_req); 
out:
	//printk("ddd setattr_after open out end\n");
	if (fault_blocked)
		filemap_invalidate_unlock(mapping);
	return 0;
		      ...
		      struct dentry *entry, umode_t mode)
{
	...
	//printk("aaarfusemkdirstart");

	if (!fm->fc->dont_mask)
		mode &= ~current_umask();

	r_req = rfuse_get_req(fm, false, false);
	//printk("aaamkdirwrong:%d",r_req->in.opcode);
	riq = rfuse_get_specific_iqueue(fm->fc, r_req->riq_id);
