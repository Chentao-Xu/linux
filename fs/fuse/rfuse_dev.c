...
static void rfuse_drop_waiting(struct fuse_conn *fc)
{
	 ...
	 */
	//printk(KERN_INFO "Thread %d waiting on conn %p\n", current->pid, fc);
	//printk("bbbrfuse_drop_waiting %d\n",atomic_read(&fc->num_waiting));
	if (atomic_dec_and_test(&fc->num_waiting) &&
			!READ_ONCE(fc->connected)) {
		//printk("bbbrfuse_drop_waiting noconnect");
		/* wake up aborters */
		wake_up_all(&fc->blocked_waitq);
	}
	//printk("bbbrfuse_drop_waiting %dafter\n",atomic_read(&fc->num_waiting));
}
...
uint32_t rfuse_get_request_buffer(struct fuse_mount *fm, int riq_id){
	...
	for(;;){
		...
		//%drequest_index,%ldargbm.bitmapp_size",request_index,riq->reqbm.bitmap_size);
		if(request_index == riq->reqbm.bitmap_size){
			//printk("cccno empty request buffer");
...
void rfuse_put_request_buffer(struct fuse_mount *fm, uint32_t request_index, int riq_id){
	...
	spin_lock(&riq->lock);
	//printk("ccc%drequest argument_bufferclear %ld",request_index,riq->reqbm.bitmap);
...
uint32_t rfuse_get_argument_buffer(struct fuse_mount *fm, int riq_id){
	...
	for(;;){
		...
		if(arg_index == riq->argbm.bitmap_size){
			// There are no empty request buffer
			//printk("cccThere are no empty arg buffer");
...
void rfuse_put_request(struct rfuse_req *r_req){
    ...
    //printk("aaawaitq has waiting processes:%d\n",waitqueue_active(&riq->waitq));	
	//printk("aaa[riq%d] Pending: %u, Completes: %u, Interrupts: %u, Forgets: %u\n",
        //    r_req->riq_id,
        //    riq->pending.tail - riq->pending.head,
        //    riq->completes.tail - riq->completes.head,
        //    riq->interrupts.tail - riq->interrupts.head,
        //    riq->forgets.tail - riq->forgets.head);
	//printk("bbbRFUSE: trying to put a RFUSE %d",r_req->riq_id);
	// if(r_req->in.opcode==0){
	// 	printk("bbbRFUSE: r_req->in.opcode==0");
	// }
	//printk("bbbRFUSE: opcode a RFUSE %d\n",r_req->in.opcode);
	if (refcount_dec_and_test(&r_req->count)) {
		//printk("bbbRFUSE: req opcode a  %d going to drop,still fc->num_background remain%d",r_req->in.opcode,fc->num_background);
		//printk("bbbstill fc->max_background %d",fc->max_background);
...
struct rfuse_req *rfuse_get_req(struct fuse_mount *fm, bool for_background, bool force){
	...
	struct rfuse_iqueue *riq;
	int err;
	//printk("dddget r_reqfirstnowwaiting%d\n",atomic_read(&fc->num_waiting));

	if(force) {
		atomic_inc(&fc->num_waiting);
		//printk("dddget r_req force\n");
		...
		if(for_background){
			...
			//printk("dddget r_req for_background force\n");
		} else{
			if(!r_req->nocreds)
		...
		if (!r_req) {
			if (for_background){
				//printk("bbbnomem wakeupblocked\n");
				wake_up(&fc->blocked_waitq);
			}
			//printk("cccrfuse_get_req and rfuse_request_alloc failed\n");		
			goto out;
		}

		// Pass riq_id to find riq if it needs to wait
		if(rfuse_block_alloc(fc, for_background, r_req->riq_id)){
			//printk("bbb ziyuan bukeyong\n");
			err = -EINTR;
			riq = rfuse_get_specific_iqueue(fc, r_req->riq_id);
		...
		//printk("aaapass riq_id to find riq\n");


		r_req->in.uid = from_kuid(fc->user_ns, current_fsuid());
		r_req->in.gid = from_kgid(fc->user_ns, current_fsgid());
		r_req->in.pid = pid_nr_ns(task_pid(current), fc->pid_ns);
		//printk("bbbr_req->in.pid = %d\n",r_req->in.pid);
...
out:
	printk("bbbr_req allocation failed\n");
	rfuse_drop_waiting(fc);
	return ERR_PTR(err);
}
...
struct rfuse_req *try_rfuse_get_req(struct fuse_mount *fm, bool for_background, bool force, spinlock_t *file_lock){
	...
	struct rfuse_iqueue *riq;
	int err;
	//printk("eeetryget r_req\n");

	if(force) {
		atomic_inc(&fc->num_waiting);
		//printk("eeetryget r_req force\n");
		...
		atomic_inc(&fc->num_waiting);
		//printk("eeetryget r_req fc->num_waiting\n");
		...
		if (unlikely(r_req->in.uid == ((uid_t)-1) || r_req->in.gid == ((gid_t)-1))) {
			//printk("eeetryget r_req unlikely(r_req->in.uid == ((uid_t)-1)\n");
			rfuse_put_request(r_req);
			return ERR_PTR(-EOVERFLOW);
		}
	}
...
out:
	printk("bbbtry r_req allocation failed\n");
	rfuse_drop_waiting(fc);
	return ERR_PTR(err);
}
...
static void rfuse_request_wait_answer(struct rfuse_req *r_req){
	...
	int i;
#endif
	//printk("aaarfuse_request_wait_answer opcode:%d",r_req->in.opcode);
	// if (fc->no_interrupt)
	// 	printk("aaarfuse_request_wait_answer opcode:%d fc->no_interrupt",r_req->in.opcode);

	if (!fc->no_interrupt) {
		//printk("aaarfuse_request_wait_answercompletion_poll");
		GET_TIMESTAMPS(1)
	...
	if (!test_bit(FR_FORCE, &r_req->flags)) {
		...
		//printk("aaarfuse_request_wait_answer can interrupt");
		...
		if (test_bit(FR_PENDING, &r_req->flags)) {
			//printk("aaarfuse_request_wait_answer can interruptnot yet in userspace");
			spin_unlock(&riq->lock);
	 ...
	 * Wait it out.
	 */
	//printk("aaarfuse_request_wait_answer must wait");
...
static void rfuse_queue_request(struct rfuse_req *r_req){
	...
	//printk("aaarfuse_queue_requestRFUSE: Send %d\n",r_req->in.opcode);
	...
	//printk("aaarfuse_queue_request: riq=%d, pending tail=%u, head=%u\n",
           //riq->riq_id, riq->pending.tail, riq->pending.head);
	
	if(waitqueue_active(&riq->idle_user_waitq)){
		//printk("aaaRFUSE:userwaitqueue_active\n");
		wake_up(&riq->idle_user_waitq);		// Wake up idle user thread
	}
	//printk("aaaRFUSE: Send %d END\n", r_req->in.opcode);
}
...
ssize_t rfuse_simple_request(struct rfuse_req *r_req){
	...
	struct fuse_mount *fm=r_req->fm;
	struct fuse_conn *fc = fm->fc;
	// if ((ret = extfuse_request_send2(fc, r_req)) != -ENOSYS)
	// 	return ret;aaa
...
static void rfuse_flush_bg_queue(struct fuse_conn *fc, int riq_id){
	...
	while (riq->active_background < riq->max_background &&
		...
		struct rfuse_req *r_req;
		//printk("eeeMOVE (BG QUEUE --> Pending QUEUE) \n");
...
static bool rfuse_request_queue_background(struct rfuse_req *r_req)
{
	...
	if (!test_bit(FR_WAITING, &r_req->flags)) {
		__set_bit(FR_WAITING, &r_req->flags);
		//printk("dddddfc->num_waitingFR_BACKGROUNDindexstart\n",r_req->index);
		atomic_inc(&fc->num_waiting);
		//printk("dddddfc->num_waitingFR_BACKGROUNDindex\n",r_req->index);
	}
	//printk("eeefc->num_waitingFR_BACKGROUNDindex",r_req->index);
...
void rfuse_request_end(struct rfuse_req *r_req){
	...
	struct rfuse_iqueue *riq = rfuse_get_specific_iqueue(fc, r_req->riq_id);
	GET_TIMESTAMPS(6)
	//printk("aaarfuse_request_endgetuserreply\n");
	if(waitqueue_active(&fc->blocked_waitq)){
		printk("bbbfc->blocked_waitq has blocked_waitq ");
	}
	if(test_bit(FR_BACKGROUND, &r_req->flags)){
		spin_lock(&riq->bg_lock);
...
static int rfuse_copy_fill(struct rfuse_copy_state *rcs)
{
	...
	if (rcs->pipebufs) {
		...
		size_t off;
		//printk("jjjjjjjj\n");
        //printk("jjjiov_iter: type=%d, count=%zu, iov_offset=%zu, nr_segs=%lu\n",
            //    iov_iter_type(rcs->iter), rcs->iter->count,
            //    rcs->iter->iov_offset, rcs->iter->nr_segs);
		...
		//printk("jjjafter iov_iter: type=%d, count=%zu, iov_offset=%zu, nr_segs=%lu\n",
            //    iov_iter_type(rcs->iter), rcs->iter->count,
            //    rcs->iter->iov_offset, rcs->iter->nr_segs);
	}
...
static int rfuse_copy_page(struct rfuse_copy_state *rcs, struct page **pagep,
	...
	while(count){
		//printk("jjjrfuse_copy_page,count:%d\n",count);
		//printk("jjjrfuse_copy_page,rcs->len:%d\n",rcs->len);
		if (rcs->write && rcs->pipebufs && page) {
			return rfuse_ref_page(rcs, page, offset, count);
		} else if (!rcs->len) {
			//printk("jjj in rfuse_copy_page,rcs->len:%d\n",rcs->len);
			if (rcs->move_pages && page && offset == 0 && count == PAGE_SIZE) {
				//printk("jjj!rcs->len,rcs->move_pages,count:%d\n",count);
				err = rfuse_try_move_page(rcs, pagep);
				//printk("jjj!rcs->len,rcs->move_pages,err%d\n",err);
				if (err <= 0)
				{
					//printk("jjj!rcs->len,rcs->move_pages,err <= 0,%d\n",err);
					return err;
				}
			} else {
				//printk("jjjbeforerfuse_copy_fill,count:%d\n",count);
...
ssize_t rfuse_dev_do_read(struct fuse_dev *fud, struct file *file, struct iov_iter *to, size_t garbage, loff_t index)
{
	...
	if (nbytes!=0)
	{
		printk("aaaaanbytesisnotnull");
	}
	else
	{
		printk("aaaaanbytesisnull");
		return 0;
	}
