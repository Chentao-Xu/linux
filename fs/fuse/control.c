...
static ssize_t fuse_conn_max_background_write(struct file *file,
  ...
  ret = fuse_conn_limit_write(file, buf, count, ppos, &val, max_user_bgreq);
  if (ret > 0) {
    struct fuse_conn *fc = fuse_ctl_file_conn_get(file);
    if (fc) {
...
static ssize_t fuse_conn_congestion_threshold_write(struct file *file,
  ...
  ret =
      fuse_conn_limit_write(file, buf, count, ppos, &val, max_user_congthresh);
  if (ret <= 0)
    goto out;
  fc = fuse_ctl_file_conn_get(file);
  if (!fc)
...
int fuse_ctl_add_conn(struct fuse_conn *fc) {
  ...
  parent =
      fuse_ctl_add_dentry(parent, fc, name, S_IFDIR | 0500, 2,
  ...
  if (!fuse_ctl_add_dentry(parent, fc, "waiting", S_IFREG | 0400, 1, NULL,
      ...
      !fuse_ctl_add_dentry(parent, fc, "abort", S_IFREG | 0200, 1, NULL,
                           &fuse_ctl_abort_ops) ||
      !fuse_ctl_add_dentry(parent, fc, "max_background", S_IFREG | 0600, 1,
                           NULL, &fuse_conn_max_background_ops) ||
      !fuse_ctl_add_dentry(parent, fc, "congestion_threshold", S_IFREG | 0600,
                           1, NULL, &fuse_conn_congestion_threshold_ops))
    goto err;
...
static void fuse_ctl_kill_sb(struct super_block *sb) {
  ...
  mutex_lock(&fuse_mutex);
  fuse_control_sb = NULL;
  list_for_each_entry(fc, &fuse_conn_list, entry) fc->ctl_ndents = 0;
  mutex_unlock(&fuse_mutex);
