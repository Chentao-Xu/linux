...
static int virtio_fs_read_tag(struct virtio_device *vdev,
                              struct virtio_fs *fs) {
  char tag_buf[sizeof_field(struct virtio_fs_config, tag)];
  char *end;
  size_t len;

  virtio_cread_bytes(vdev, offsetof(struct virtio_fs_config, tag), &tag_buf,
                     sizeof(tag_buf));
...
static void virtio_fs_hiprio_done_work(struct work_struct *work) {
  struct virtio_fs_vq *fsvq =
      container_of(work, struct virtio_fs_vq, done_work);
...
static void virtio_fs_request_dispatch_work(struct work_struct *work) {
  struct fuse_req *req;
  struct virtio_fs_vq *fsvq =
      container_of(work, struct virtio_fs_vq, dispatch_work.work);
  int ret;

  pr_debug("virtio-fs: worker %s called.\n", __func__);
  while (1) {
    spin_lock(&fsvq->lock);
    req = list_first_entry_or_null(&fsvq->end_reqs, struct fuse_req, list);
    if (!req) {
      spin_unlock(&fsvq->lock);
      break;
    }
  ...
  while (1) {
    spin_lock(&fsvq->lock);
    req = list_first_entry_or_null(&fsvq->queued_reqs, struct fuse_req, list);
    if (!req) {
      spin_unlock(&fsvq->lock);
      return;
    }
    ...
    if (ret < 0) {
      if (ret == -ENOMEM || ret == -ENOSPC) {
        ...
        schedule_delayed_work(&fsvq->dispatch_work, msecs_to_jiffies(1));
        spin_unlock(&fsvq->lock);
        return;
      }
      req->out.h.error = ret;
...
static int copy_args_to_argbuf(struct fuse_req *req) {
  ...
  for (i = 0; i < num_in; i++) {
    memcpy(req->argbuf + offset, args->in_args[i].value, args->in_args[i].size);
...
static void copy_args_from_argbuf(struct fuse_args *args,
                                  struct fuse_req *req) {
  ...
  for (i = 0; i < num_out; i++) {
    ...
    if (args->out_argvar && i == args->out_numargs - 1 && argsize > remaining) {
      argsize = remaining;
    }
...
static void virtio_fs_complete_req_work(struct work_struct *work) {
  struct virtio_fs_req_work *w = container_of(work, typeof(*w), done_work);
...
static void virtio_fs_init_vq(struct virtio_fs_vq *fsvq, char *name,
  ...
  if (vq_type == VQ_REQUEST) {
    ...
    INIT_DELAYED_WORK(&fsvq->dispatch_work, virtio_fs_hiprio_dispatch_work);
  }
}
...
static int virtio_fs_setup_vqs(struct virtio_device *vdev,
  ...
  callbacks = kmalloc_array(fs->nvqs, sizeof(callbacks[VQ_HIPRIO]), GFP_KERNEL);
...
static long virtio_fs_direct_access(struct dax_device *dax_dev, pgoff_t pgoff,
  ...
  if (pfn)
    *pfn = phys_to_pfn_t(fs->window_phys_addr + offset, PFN_DEV | PFN_MAP);
  return nr_pages > max_nr_pages ? max_nr_pages : nr_pages;
}

static int virtio_fs_zero_page_range(struct dax_device *dax_dev, pgoff_t pgoff,
                                     size_t nr_pages) {
  long rc;
  void *kaddr;
...
static int virtio_fs_setup_dax(struct virtio_device *vdev,
  ...
  /* Get cache region */
  have_cache =
      virtio_get_shm_region(vdev, &cache_reg, (u8)VIRTIO_FS_SHMCAP_ID_CACHE);
  if (!have_cache) {
...
static unsigned int sg_count_fuse_req(struct fuse_req *req) {
  ...
  if (args->in_pages) {
    ...
    total_sgs += sg_count_fuse_pages(ap->descs, ap->num_pages, size);
  }
...
static unsigned int sg_init_fuse_args(struct scatterlist *sg,
                                      ...
                                      unsigned int numargs, bool argpages,
                                      void *argbuf, unsigned int *len_used) {
  ...
  if (argpages)
    total_sgs += sg_init_fuse_pages(&sg[total_sgs], ap->pages, ap->descs,
                                    ap->num_pages, args[numargs - 1].size);
...
static int virtio_fs_enqueue_req(struct virtio_fs_vq *fsvq,
  ...
  out_sgs += sg_init_fuse_args(
      &sg[out_sgs], req, (struct fuse_arg *)args->in_args, args->in_numargs,
      args->in_pages, req->argbuf, &argbuf_used);

  /* Reply elements */
  if (test_bit(FR_ISREPLY, &req->flags)) {
    sg_init_one(&sg[out_sgs + in_sgs++], &req->out.h, sizeof(req->out.h));
    in_sgs += sg_init_fuse_args(&sg[out_sgs + in_sgs], req, args->out_args,
                                args->out_numargs, args->out_pages,
