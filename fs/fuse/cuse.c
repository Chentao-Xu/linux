...
static ssize_t cuse_write_iter(struct kiocb *kiocb, struct iov_iter *from) {
   ...
   */
  return fuse_direct_io(&io, from, &pos, FUSE_DIO_WRITE | FUSE_DIO_CUSE);
}
...
static int cuse_open(struct inode *inode, struct file *file) {
  ...
  list_for_each_entry(pos, cuse_conntbl_head(devt),
                      list) if (pos->dev->devt == devt) {
...
static void cuse_process_init_reply(struct fuse_mount *fm,
  ...
  for (i = 0; i < CUSE_CONNTBL_LEN; ++i) {
    list_for_each_entry(
        pos, &cuse_conntbl[i],
        list) if (!strcmp(dev_name(pos->dev), dev_name(dev))) goto err_unlock;
  }
...
static int cuse_channel_open(struct inode *inode, struct file *file) {
   ...
   */
  fuse_conn_init(&cc->fc, &cc->fm, file->f_cred->user_ns, &fuse_dev_fiq_ops,
                 NULL);
