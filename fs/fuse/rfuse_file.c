...
int rfuse_flush(struct file *file, fl_owner_t id)
{
	...
	if (err){
		//printk("eeerfuse_flush filemap_check_errors\n");
		//printk("eeerfuse_flush filemap_check_errors:  %d\n",file->f_mapping->wb_err);
		return err;
	}
		
	err = 0;
	if (fm->fc->no_flush){
		//printk("eeerfuse_flush fm->fc->no_flush");//ddd
		goto inval_attr_out;
	}//ddd
	//printk("eeerfuse_flush before rfuse_get_reqend\n");
	...
	if (err == -ENOSYS) {
		//printk("eeerfuse_flush fm->fc->no_flush set 1");
		fm->fc->no_flush = 1;
		err = 0;
	}
...
static int rfuse_send_open(struct fuse_mount *fm, u64 nodeid,
	...
	if (fm->fc->atomic_o_trunc)
		//printk("eeerfuse_send_open check fm->fc->atomic_o_trunc on\n ");
	if (!fm->fc->atomic_o_trunc)
		inarg->flags &= ~O_TRUNC;
				 ...
				 unsigned int open_flags, bool isdir)
{
	...
	//printk("eeerfuse_file_openstart");
...
void rfuse_file_release(struct inode *inode, struct fuse_file *ff,
	...
	if(ff->fm->fc->destroy){
		//printk("eee ff->fm->fc->destroy true,not backgroud\n");
		r_req = rfuse_get_req(fm, false, true);
	}
	else
	{
		//printk("eee ff->fm->fc->destroy false,backgroud\n");
			       ...
			       unsigned int max_pages)
{
	...
	//printk("rfuse_get_user_pages: nbytesp : %ld, max_pages: %d\n", *nbytesp, max_pages);
		       ...
		       loff_t *ppos, int flags)
{
	...
	struct rfuse_io_args *ria;
	unsigned int max_pages;
	//printk("dddrfuse_direct_io: count=%zu, fc->max_pages=%u\n", count, fc->max_pages);
	max_pages = iov_iter_npages(iter, fc->max_pages);
	//printk("jjjrfuse_direct_io: count=%zu, max_pages=%u, fc->max_pages=%u\n", count, max_pages, fc->max_pages);
	...
	while (count) {
		...
		//printk("jjjrfuse_direct_io:nbytes=%zu, max_pages=%u, fc->max_pages=%u\n", nbytes, max_pages, fc->max_pages);
		...
		if (count) {
			...
			//printk("jjjrfuse_direct_io:count=%zu, max_pages=%u, fc->max_pages=%u\n", count, max_pages, fc->max_pages);
			ria = rfuse_io_alloc(io, max_pages);
			if (max_pages == 0 && count > 0) {
	//printk("whyzeromax_pages\n");
    rfuse_io_free(ria);
    return -1; // 或其他合适的错误
}
			if (!ria)
				     ...
				     struct iov_iter *ii, loff_t pos, unsigned int max_pages)
{
	...
	size_t count = 0;
	int err;
	//printk("dddwhyria->r_reqnull%p",ria->r_req);
	//printk("dddrfuse_fill_write_pagesherewrong");
...
ssize_t rfuse_perform_write(struct kiocb *iocb, struct address_space *mapping, struct iov_iter *ii, loff_t pos){
	...
	int err = 0;
	ssize_t res = 0;
	//printk("dddrfuse_perform_writestart");
	...
	do {
		...
		//printk("dddrfuse_perform_writedo");
...
__acquires(fi->lock)
{
	...
	int err;
	//printk("eeerfuse_send_writepage start\n");
	...
	//printk("eeeinarg->offset:%d data_size:%d size:%dstart\n",inarg->offset,data_size,size);
	if (inarg->offset + data_size <= size) {
		inarg->size = data_size;
	} else if (inarg->offset < size) {
		inarg->size = size - inarg->offset;
	} else {
		//printk("eeewrong free rfuse_send_writepage\n");
	...
	//printk("eeerfuse_send_writepage rfuse_simple_background");
...
__acquires(fi->lock)
{
	...
	struct rfuse_writepage_args *r_wpa;
	//printk("dddrfuse_flush_writepages start\n");
	//printk("dddrfuse_flush_writepages start fi->writectr%d\n",fi->writectr);
	// if(list_empty(&fi->queued_writes)){
	// 	printk("dddrfuse_flush_writepages start queued_writes zero\n");
	// }
	

	while (fi->writectr >= 0 && !list_empty(&fi->queued_writes)) {
		//printk("dddrfuse_flush_writepages real write start\n");
...
static void rfuse_writepages_send(struct rfuse_fill_wb_data *data)
{
	...
	int i;
	//printk("bbb rfuse_writepages_send start\n");
	...
	//printk("bbb rfuse_writepages_send end\n");
}
		...
		struct writeback_control *wbc, void *_data)
{
	...
	struct page *tmp_page;
	int err;
	//printk("fffrfuse_writepages_fillstart\n");
...
static void rfuse_send_readpages(struct rfuse_io_args *ria, struct file *file){
	...
	ria->r_req = r_req;
	//printk("eeerfuse_send_readpages start\n");
